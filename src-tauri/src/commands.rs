...
#[tauri::command]
pub async fn test_fullscreen_overlay(app: AppHandle) -> Result<String, String> {
    tracing::info!("🧪 开始测试全屏覆盖功能");
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        // 1. 检测全屏状态
        let fullscreen_result = macos_window::detect_fullscreen_app()?;
        tracing::info!("📊 全屏检测结果: {}", fullscreen_result);
        
        // 2. 测试智能显示窗口
        macos_window::show_window_smart(&app)?;
        
        let result_message = if fullscreen_result.starts_with("fullscreen:") {
            let app_name = fullscreen_result.strip_prefix("fullscreen:").unwrap_or("Unknown");
            format!("✅ 检测到全屏应用: {}，已使用覆盖模式显示窗口", app_name)
        } else {
            "✅ 无全屏应用，已使用普通模式显示窗口".to_string()
        };
        
        tracing::info!("🎉 测试完成: {}", result_message);
        Ok(result_message)
    }
    ...
    {
        let result_message = "ℹ️ 此功能仅在 macOS 平台可用";
        tracing::info!("{}", result_message);
        Ok(result_message.to_string())
    }
}

#[tauri::command]
pub async fn test_window_overlay_directly(app: AppHandle) -> Result<String, String> {
    tracing::info!("🧪 直接测试窗口覆盖功能");
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        // 直接使用最高级别显示窗口
        macos_window::show_window_on_top(&app)?;
        
        let result_message = "✅ 窗口已设置为屏保级别，可覆盖全屏应用";
        tracing::info!("🎉 {}", result_message);
        Ok(result_message.to_string())
    }
